...
export class ClientV1UserController {
  ...
  public async UpdateUserLocation(
    @Headers() { language }: GetLanguageDto,
    @Headers() {}: GetVersionDto,
    @Body() user_location: UpdateUserLocationRequestDto,
    @RestAuth() auth: JwtPayloadInterface,
  ): Promise<UpdateUserLocationResponseDto> {
    return this.registerUserUseCase.UpdateUserLocation(auth.user.user_id, user_location, language);
  }
}
