...
export class RegisterUserUseCase {
  ...
  public async UpdateUserLocation(
    user_id: number,
    user_location: UpdateUserLocationInterface,
    lang: string,
  ): Promise<UpdateUserLocationResponse> {
    try {
      const is_updated = await this.commandBus.execute(new UpdateUserLocationCommand(user_id, user_location, lang));

      return {
        data: is_updated,
        meta: {
          ...localiczation.message(LocalizationMessage.UPDATING_USER_LOCATION_SUCCESSFULLY, { lang }),
        },
      };
    } catch (error) {
      if (error?.response?.meta) throw error;
